...
| EvoZeus-wrapper | `10-repos/EvoZeus-wrapper` | EvoZeus-orchestrated static SKILL wrapper harness / submodule | public | public | MetaInFlow | active seed / 已创建 / 已接入 | 由 EvoZeus 母体调度，负责把 promoted 或已有静态 `SKILL.md` repo 化，并补齐 feedback、design doc、PR、CHANGELOG、release 和 preflight 闭环；不拥有 runtime 执行、scanner、ledger、Session Signal 方法或 official factor tools。Maintain: `evozeus-maintainers`；涉及执行能力必须路由到 `evozeus-infra`。 |
