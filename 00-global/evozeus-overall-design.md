...
flowchart TB
  ...
  Mega --> Runtime["evozeus-runtime<br/>local execution kernel / CLI / TUI / scanner / runner"]
...
| `evozeus-runtime` | local execution kernel：workspace、scanner sandbox、Python factor runner、SQLite ledger、report、CLI/TUI/companion | active shell / 产品能力仍需实现 |
...
| scanner / resolver / local execution / SQLite / report generation | `evozeus-runtime` |
...
| Runtime execution | `evozeus-runtime` | 按 Python contract 运行 selected factors，写 ledger/report | 不定义公共协议语义 |
...
| `evozeus-runtime` | private | user-installable runtime 前 public | permission model、upload-off-by-default、scanner sandbox、lockfile、dependency audit |
...
| `evozeus-runtime-maintainers` | local execution kernel 维护 |
...
- `evozeus-runtime` 先补 workspace/config/lockfile，再补 SQLite ledger。
